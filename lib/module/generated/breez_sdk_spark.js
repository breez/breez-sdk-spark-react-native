 ...
 */export const LeafOptimizationConfig=(()=>{const defaults=()=>({});const create=(()=>{return uniffiCreateRecord(defaults);})();return Object.freeze({/**
 ...
 */export const TokenOptimizationConfig=(()=>{const defaults=()=>({});const create=(()=>{return uniffiCreateRecord(defaults);})();return Object.freeze({/**
...
constructor(pointer){super();this[pointerLiteralSymbol]=pointer;this[destructorGuardSymbol]=uniffiTypeSdkContextObjectFactory.bless(pointer);}/**
   * {@inheritDoc uniffi-bindgen-react-native#UniffiAbstractObject.uniffiDestroy}
   */uniffiDestroy(){const ptr=this[destructorGuardSymbol];if(ptr!==undefined){const pointer=uniffiTypeSdkContextObjectFactory.pointer(this);uniffiTypeSdkContextObjectFactory.freePointer(pointer);uniffiTypeSdkContextObjectFactory.unbless(ptr);delete this[destructorGuardSymbol];}}static instanceOf(obj){return uniffiTypeSdkContextObjectFactory.isConcreteType(obj);}}const uniffiTypeSdkContextObjectFactory=(()=>{return{create(pointer){const instance=Object.create(SdkContext.prototype);instance[pointerLiteralSymbol]=pointer;instance[destructorGuardSymbol]=this.bless(pointer);instance[uniffiTypeNameSymbol]='SdkContext';return instance;},bless(p){return uniffiCaller.rustCall(/*caller:*/status=>nativeModule().ubrn_uniffi_internal_fn_method_sdkcontext_ffi__bless_pointer(p,status),/*liftString:*/FfiConverterString.lift);},unbless(ptr){ptr.markDestroyed();},pointer(obj){if(obj[destructorGuardSymbol]===undefined){throw new UniffiInternalError.UnexpectedNullPointer();}return obj[pointerLiteralSymbol];},clonePointer(obj){const pointer=this.pointer(obj);return uniffiCaller.rustCall(/*caller:*/callStatus=>nativeModule().ubrn_uniffi_breez_sdk_spark_fn_clone_sdkcontext(pointer,callStatus),/*liftString:*/FfiConverterString.lift);},freePointer(pointer){uniffiCaller.rustCall(/*caller:*/callStatus=>nativeModule().ubrn_uniffi_breez_sdk_spark_fn_free_sdkcontext(pointer,callStatus),/*liftString:*/FfiConverterString.lift);},isConcreteType(obj){return obj[destructorGuardSymbol]&&obj[uniffiTypeNameSymbol]==='SdkContext';}};})();// FfiConverter for SdkContextInterface
const FfiConverterTypeSdkContext=new FfiConverterObject(uniffiTypeSdkContextObjectFactory);/**
